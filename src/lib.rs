...
/// Probes are short-lived checks with nothing to clean up, and a probe that is being stopped has
/// already been given its own `timeout`, so the grace period is deliberately much shorter than a
/// task's.
