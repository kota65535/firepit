...
impl<W: Write> Write for PrefixedWriter<W> {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        for chunk in buf.split_inclusive(|c| *c == b'\r') {
            ...
            // of the line so we want to rewrite the prefix over the existing prefix in the line or
            // if the last chunk is just a newline we can skip rewriting the prefix
            if is_first || chunk != b"\n" {
        ...
        // We do end up writing more bytes than this to the underlying writer, but we cannot report
        // this to the callers as the amount of bytes we report written must be less than or equal
        // to the number of bytes in the buffer.
        Ok(buf.len())
    }
