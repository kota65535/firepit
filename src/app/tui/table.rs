...
impl<'a> StatefulWidget for &'a TaskTable<'a> {
    ...
    fn render(self, area: Rect, buf: &mut ratatui::prelude::Buffer, state: &mut Self::State) {
        ...
        let separator_line = |indicator: Option<&str>| -> String {
            match indicator {
                Some(ch) => {
                    let indicator_width = 2; // e.g. "⌃⌃"
                    let left = width.saturating_sub(indicator_width) / 2;
                    let right = width.saturating_sub(left + indicator_width);
                    format!(
                        "{}{}{}",
                        "─".repeat(left),
                        ch.repeat(2),
