...
mod test {
    ...
    use crate::ui::lib::{BOLD, MAGENTA};
    use super::*;

    fn prefixed_ui<W: Write>(out: W, err: W, color_config: ColorConfig) -> PrefixedUI<W> {
        let output_prefix = BOLD.apply_to("output ".to_string());
        let warn_prefix = MAGENTA.apply_to("warn ".to_string());
        PrefixedUI::new(color_config, out, err)
            .with_output_prefix(output_prefix)
            .with_warn_prefix(warn_prefix)
            .with_error_prefix(MAGENTA.apply_to("error ".to_string()))
    }
